...
func HashHTTPBody(node *pb.HTTPBody) []byte {
	...
	if node.OtherType != nil {
		hash.Write(intHashes[2])
		hash.Write(HashString(node.OtherType))
	}
	return hash.Sum(nil)
}
...
var ProtobufFileHashes map[string][]byte = map[string][]byte{"method.proto": []byte{90, 163, 173, 15, 200, 83, 168, 153}, "witness.proto": []byte{42, 213, 185, 25, 124, 226, 76, 187}, "types.proto": []byte{98, 84, 34, 180, 249, 140, 214, 227}, "spec.proto": []byte{13, 101, 129, 126, 232, 252, 1, 146}}
