...
func TestSummarizeWithResponseCodeFilter(t *testing.T) {
	expected := &Summary{
		Authentications: map[string]int{
			"BASIC": 1,
		},
	...
	m1 := test.LoadMethodFromFileOrDie("testdata/method1.pb.txt")
	m2 := test.LoadMethodFromFileOrDie("testdata/method2.pb.txt")
	methods := &pb.APISpec{Methods: []*pb.Method{m1, m2}}
	filters := map[string][]string{"response_codes": {"200"}}
	assert.Equal(t, expected, SummarizeWithFilters(methods, filters))
}

func TestSummarizeWithDirection(t *testing.T) {
	expected := &Summary{
		Authentications: map[string]int{
			"BASIC": 2,
		},
	...
	m2 := test.LoadMethodFromFileOrDie("testdata/method2.pb.txt")
	methods := &pb.APISpec{Methods: []*pb.Method{m1, m2}}
	filters := map[string][]string{"directions": {"request"}}
	assert.Equal(t, expected, SummarizeWithFilters(methods, filters))
}
